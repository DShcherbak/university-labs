...
void Widget::mousePressEvent(QMouseEvent *event){
    ...
    if(currentState == NodesBuilding && y <= 530){
       ...
       //     }
        ...
        boundaries = steps_of_Jarvice[id];
        if(id < steps_of_Jarvice.size() - 1){
            id++;
        }
        update();
    }
...
void Widget::on_pushButton_2_clicked()
{
    ...
    steps_of_Jarvice = DivideAndConquere(nodes);//Jarvice(nodes, nodes[min_x], nodes[max_x], 1);
